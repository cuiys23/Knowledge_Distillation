...
The cross-entropy loss is used for this objective:
\begin{equation}
...
Since direct one-to-one alignment between teacher and student feature representations lacks theoretical grounding, we incorporate a regressor between them~\cite{romero2014fitnets}, enabling the student to learn more flexible representations rather than naively copying the teacher.
...
\end{equation}
where ${\lambda _o} > 0$ denotes the weighting parameter employed to balance performance on previous and new fault classes.
...
The total training objective combines the classification and distillation losses:
\begin{equation}
...
Following~\cite{li2020federated}, we introduce a proximal regularization term to mitigate the impact of data heterogeneity among TT\&C centers. The final objective function of the algorithm is expressed as follows:
\begin{equation}
...
$\mu$ is increased when the objective function rises and decreased when it falls.
...
This design strengthens weaker performers and thereby enhances overall model robustness.
