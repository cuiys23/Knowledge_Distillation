...
Weights \(\alpha_{k,t}, \beta_t\) follow dynamic weighting and the proxy schedule \(\gamma_t^{\mathrm{proxy}} = \bar C_\gamma (t+1)^{-(1+\rho)}\;(\rho > 0)\) where \(\bar C_\gamma > 0\) is the initial proxy weight coefficient.
Concretely, \(\alpha_{k,t} = \phi[\kern-0.15em[ \pi_k \gamma^k ]\kern-0.15em]\) with \(\phi\) a normalized mapping (see Section~\ref{sec:proposed_framework}-\ref{section:3-2}-\ref{section:3-2-2}), and \(\beta_t = \pi_{\mathrm{proxy}}\gamma_t^{\mathrm{proxy}}\).
(Note: \(\gamma^k\) is itself a softmax-normalized importance factor from Algorithm~\ref{Alg:Dynamic_Weighting_Method}; \(\phi\) provides re-normalization over the expanded set \(S_t \cup \{\mathrm{proxy}\}\).)

\noindent\textbf{Notation bridge to the algorithm description.}
The analysis employs time-indexed notation that generalizes the algorithm's constants:
\(\eta_t\) corresponds to the local step size (denoted \(\eta\) in Algorithm~\ref{Alg:FL-PFC}),
\(\mu_t\) and \(\lambda_t^k\) generalize the constants \(\mu\) and \(\lambda^i\) appearing in Algorithm~\ref{Alg:Inheritance_Distillation_Method},
and \(\omega_t^{\mathrm{proxy}}\) denotes the proxy model stored at the virtual client (\(\theta_\times^*\) in Section~\ref{sec:proposed_framework}-\ref{section:3-2}-\ref{section:3-2-1}).
The per-round variation of \(\lambda_t^k,\mu_t\) (due to adaptive adjustment) is captured by the drift analysis in Assumption~\ref{asmp:4}--Proposition~\ref{prop:4}.
...
\begin{assumption}[Heterogeneity and Boundedness]\label{asmp:2}
  ...
  Every round \(S_t \neq \varnothing\) and \(\pi_k \ge \pi_{\min} > 0\) where \(\pi_k = p_k = n_k/n\) and \(\pi_{\mathrm{proxy}}\) denotes the data proportion of the communication-constrained (proxy) client.
\end{assumption}

\begin{assumption}[Algorithmic Bridging]\label{asmp:3}
...
\begin{proposition}[Compactness and Projection]\label{prop:1}
  If a compact set \(\mathcal K_\omega\) contains all \(\omega_t, \omega_{t,e}^k, \omega_t^{\mathrm{proxy}}\) and the stochastic gradient map is continuous, then Assumption~\ref{asmp:3} holds (with \(G\) equal to the maximum gradient norm on the compact set and \(R_\beta = \operatorname{diam}(\mathcal K_\omega)\)).
  With projection-clipping and Euclidean projection, boundedness conditions in Assumption~\ref{asmp:2} also hold.
\end{proposition}
...
\begin{proposition}[Bias Decomposition]\label{prop:2}
...
\begin{proposition}[Proxy Residual]\label{prop:3}
...
\begin{proof}[Proof sketch]
  ...
  Combining, weighting by \(p_k\), and applying the heterogeneity assumption~\ref{asmp:2} gives
...
\begin{theorem}[Surrogate Objective Convergence]\label{thm:1}
  ...
  Under Assumptions~\ref{asmp:1}, \ref{asmp:2}, and \ref{asmp:3}:
  \begin{align}
...
\begin{assumption}[Regularity and Drift]\label{asmp:4}
  \(\widetilde F_{k,t}\) is \(L\)-smooth with the same heterogeneity bound as Assumption~\ref{asmp:2};
...
\begin{proposition}[Sufficient Drift Condition]\label{prop:4}
  ...
  then the drift condition in Assumption~\ref{asmp:4} holds with
...
\begin{proposition}[Lemma Transfer]\label{prop:5}
...
\begin{theorem}[Adaptive Objective Convergence]\label{thm:2}
  ...
  Assumptions~\ref{asmp:2}, \ref{asmp:3}, and \ref{asmp:4}:
  \begin{align}
...
\begin{proof}[Proof sketch]
  By Proposition~\ref{prop:5}, Eq.\,\eqref{eq:33} holds for \(\widetilde F_t\).
  Using \(\widetilde F_{t+1}(\omega_{t+1}) \le \widetilde F_t(\omega_{t+1}) + \varepsilon_t\) from Assumption~\ref{asmp:4},
...
\(G\) (Assumption~\ref{asmp:3}) is the maximum client gradient norm, analogous to a fixed noise floor in communication systems, determining the ultimate accuracy ceiling.
